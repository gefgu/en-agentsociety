...
const Page = () => {
  ...
  const setupRefreshTimer = (interval: number, expId: string) => {
    clearRefreshTimer();
    if (interval > 0) {
      refreshTimerRef.current = setInterval(() => fetchLog(expId), interval * 1000);
    }
  };
  ...
  const handleStatusChip = (s: number | null) => { setStatusFilter(s); fetchData(idFilter, nameFilter, s); };
  const handleSearch = () => fetchData();
  const handleReset = () => { setIdFilter(''); setNameFilter(''); setStatusFilter(null); fetchData('', '', null); };

  const columns: ColumnsType<Experiment> = [
    {
      title: t('console.table.id'), dataIndex: 'id', width: 140,
      render: (id: string) => <MonoCell style={{ fontSize: 10.5 }}>{id ? id.slice(0, 8) + '…' : '—'}</MonoCell>,
    },
    {
      title: t('console.table.name'), dataIndex: 'name',
      render: (name: string) => <span className="console-exp-name">{name}</span>,
    },
    { title: t('console.table.numDay'), dataIndex: 'num_day', width: 70 },
    {
      title: t('console.table.status'), dataIndex: 'status', width: 120,
      render: (status: number) => <StatusBadge status={status} label={t(`console.statusEnum.${status}`)} />,
    },
    { title: t('console.table.currentDay'), dataIndex: 'cur_day', width: 70 },
    {
      title: t('console.table.currentTime'), dataIndex: 'cur_t', width: 80,
      render: (v: number) => <MonoCell>{parseT(v)}</MonoCell>,
    },
    {
      title: t('console.table.inputTokens'), dataIndex: 'input_tokens', width: 90,
      render: (v: number) => <MonoCell>{formatTokens(v)}</MonoCell>,
    },
    {
      title: t('console.table.outputTokens'), dataIndex: 'output_tokens', width: 90,
      render: (v: number) => <MonoCell>{formatTokens(v)}</MonoCell>,
    },
    {
      title: t('console.table.createdAt'), dataIndex: 'created_at', width: 150,
      render: (v: string) => <MonoCell style={{ fontSize: 10 }}>{v ? new Date(v).toLocaleString() : '—'}</MonoCell>,
    },
    {
      title: t('console.table.updatedAt'), dataIndex: 'updated_at', width: 150,
      render: (v: string) => <MonoCell style={{ fontSize: 10 }}>{v ? new Date(v).toLocaleString() : '—'}</MonoCell>,
    },
    {
      title: t('console.table.action'), width: 160,
      render: (_, record) => {
        record = { ...record };
        return (
          <Space size={4}>
            <Button type="primary" size="small" style={{ fontSize: 11 }}
              onClick={() => navigate(`/exp/${record.id}`)} disabled={record.status === 0}>
              ...
              <Popconfirm
                ...
                onConfirm={async () => {
                  try {
                    ...
                    if (res.ok) { message.success(t('console.messages.stopSuccess')); fetchData(); }
                    else throw new Error(await res.text());
                  } catch (err) { message.error(t('console.messages.stopFailed') + err); }
                }}
              >
                <Button danger size="small" className="console-btn-stop">{t('console.buttons.stop')}</Button>
              </Popconfirm>
            )}
            <Dropdown
              menu={{
                      ...
                      <Popconfirm
                        ...
                        onConfirm={async () => {
                          try {
                            ...
                            if (res.ok) { message.success(t('console.messages.deleteSuccess')); fetchData(); }
                            else throw new Error(await res.text());
                          } catch (err) { message.error(t('console.messages.deleteFailed') + err); }
                        }}
                      >
                        <span className="console-delete-label">{t('console.buttons.delete')}</span>
                      </Popconfirm>
                    ),
                  },
                ],
    ...
    <>
      <div className="console-page">
        <div className="console-header">
          <div>
            <h1 className="console-title">{t('console.pageTitle')}</h1>
            <p className="console-subtitle">{t('console.pageSubtitle')}</p>
          </div>
          <Button type="primary" icon={<PlusOutlined />} onClick={() => navigate('/create-experiment')}
        ...
        <div className="console-filter-bar">
          <span className="console-filter-label">FILTER</span>
          <Input placeholder={t('console.table.id')} value={idFilter} onChange={e => setIdFilter(e.target.value)}
            style={{ width: 160 }} allowClear />
          <Input placeholder={t('console.table.name')} value={nameFilter} onChange={e => setNameFilter(e.target.value)}
            style={{ width: 200 }} allowClear />
          <Select value={statusFilter} onChange={v => setStatusFilter(v)} style={{ width: 150 }}
            allowClear placeholder={t('console.table.status')}
            options={[0,1,2,3,4].map(v => ({ value: v, label: t(`console.statusEnum.${v}`) }))} />
          <Button onClick={handleReset}>{t ? 'Reset' : 'Reset'}</Button>
        ...
        <div className="console-chips">
          {statusChips.map(({ status, key }) => (
            <StatusChip key={key} status={status}
        ...
        <div className="console-table-wrap">
          <Table<Experiment>
            dataSource={tableData} columns={columns} loading={tableLoading}
            rowKey="id" pagination={{ pageSize: 20, size: 'small' }}
            size="small" scroll={{ x: 1200 }} style={{ background: 'transparent' }}
          />
        </div>
      </div>

      <Modal title={t('console.modals.experimentDetail')} width="60vw"
        open={detail !== null} onCancel={() => setDetail(null)} footer={null}>
        <ProDescriptions<Experiment>
          column={2} title={detail?.name}
          ...
          columns={[
            ...
            { title: t('console.table.status'), dataIndex: 'status', render: (s: number) => t(`console.statusEnum.${s}`) },
      ...
      <Modal title={t('console.modals.experimentLog')} width="80vw" open={logVisible}
        onCancel={() => { setLogVisible(false); setLogContent(''); setRefreshInterval(0); clearRefreshTimer(); }}
        footer={null}>
        <div className="console-log-toolbar">
          <Button icon={<ReloadOutlined />} onClick={() => fetchLog(currentExpId)} loading={logLoading}>
            {t('console.modals.refresh')}
          </Button>
          <Select value={refreshInterval}
            onChange={v => { setRefreshInterval(v); setupRefreshTimer(v, currentExpId); }}
            style={{ width: 200 }}
            options={[
          ...
          />
          {logLoading && <span className="console-log-refreshing">{t('console.modals.refreshing')}</span>}
        </div>
        <pre className="console-log-pre">{logContent}</pre>
      </Modal>
    </>
  );
};
